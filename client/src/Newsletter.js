...
const NewsletterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('https://kyrix.dev/ainexus/api/submit-email', {
        method: 'POST',
            ...
            <div className="newsletter-inner">
              <div className="newsletter-title">
                ...
                <p className="text-gray-400">
                  Want to stay updated on the latest tools and tutorials 📣? Sign up for our newsletter and never miss an update!
                </p>
              </div>
              <div className="newsletter-form">
                <form onSubmit={handleSubmit} method="POST">
                  ...
                  />
                  <br />
                  <button type="submit" className="lg:mx-6 Btn-color block mx-auto" disabled={isLoading}>
                    {isLoading ? 'Submitting...' : 'Subscribe'}
                  </button>
                  {error && (
                    <p className="bg-stone-900 w-fit p-2 rounded-lg font-medium my-4 px-6 py-2 text-base">
                      {error}
                    </p>
                  )}
                </form>
