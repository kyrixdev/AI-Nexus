...
const SearchBar = ({searchQuery, setSearchQuery, originalList}) => {
  const handleSearchChange = (e) => {
    if (e.target.value === '') {
        setSearchQuery(originalList);
    } else {
        const filteredTools = originalList.filter(tool => tool.name.toLowerCase().includes(e.target.value.toLowerCase()) || tool.description.toLowerCase().includes(e.target.value.toLowerCase()))
